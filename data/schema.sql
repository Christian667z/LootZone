...
-- Lecture : tout staff authentifié (pas les anonymes)
drop policy if exists "Staff peut lire les profils" on profiles;
create policy "Staff peut lire les profils"
  on profiles for select
  using (auth.uid() is not null);

-- Mise à jour : chacun modifie seulement son propre profil
drop policy if exists "Staff peut modifier son propre profil" on profiles;
create policy "Staff peut modifier son propre profil"
  on profiles for update
  using (auth.uid() = id);

-- Insertion : uniquement les utilisateurs authentifiés (backend service_role)
...
-- Elle ne doit être appelée que par le système auth de Supabase, jamais via RPC
